...
// Umbral: si el texto extraído de un PDF tiene menos caracteres, se considera escaneado
const MIN_CHARS_PDF_TEXTO = 80;

const PROMPT_BASE = `Extrae los datos de este documento y devuelve ÚNICAMENTE un objeto JSON válido, sin texto adicional, con esta estructura exacta:
{
...
function construirPrompt(ejemplos) {
  if (!ejemplos || ejemplos.length === 0) return PROMPT_BASE;
  const lineas = ejemplos.map(e => {
    const campos = {
      tipo:           e.tipo,
      ...
      concepto:       e.concepto,
      base_imponible: e.base_imponible != null ? Number(e.base_imponible) : null,
      iva_porcentaje: e.iva_porcentaje != null ? Number(e.iva_porcentaje) : null,
      iva_importe:    e.iva_importe    != null ? Number(e.iva_importe)    : null,
      importe_total:  e.importe        != null ? Number(e.importe)        : null,
    };
    return JSON.stringify(campos);
  });
  return `${PROMPT_BASE}\n\nEjemplos de facturas ya revisadas y confirmadas (úsalos para calibrar tu extracción):\n${lineas.join('\n')}`;
}

function parsearRespuestaJson(texto) {
  const match = texto.match(/\{[\s\S]*\}/);
  try { return JSON.parse(match ? match[0] : '{}'); } catch (_) { return {}; }
}
...
async function extraerConKimi(rutaArchivo, mimeType, prompt) {
  const esPdf = mimeType === 'application/pdf';
  let respuesta;

  if (esPdf) {
    const texto = await extraerTextoPdfKimi(rutaArchivo, path.basename(rutaArchivo));
    // PDF escaneado: sin texto extraíble → Kimi no puede procesarlo
    if (texto.trim().length < MIN_CHARS_PDF_TEXTO) {
      throw new Error('PDF escaneado (sin texto extraíble), Kimi no puede procesarlo');
    }
  ...
  return { ocrRawJson: respuesta, extraido: parsearRespuestaJson(texto) };
}
...
async function extraerConGemini(rutaArchivo, mimeType, prompt) {
  // Gemini soporta application/pdf nativamente (incluyendo PDFs escaneados)
  const b64   = fs.readFileSync(rutaArchivo).toString('base64');
...
// ── Punto de entrada con few-shot y fallback ──────────────────────────────────

async function extraerDatosFactura(rutaArchivo, mimeType, ejemplos = []) {
  const prompt = construirPrompt(ejemplos);

  if (process.env.MOONSHOT_API_KEY) {
    try {
      const resultado = await extraerConKimi(rutaArchivo, mimeType, prompt);
      return { ...resultado, proveedor_ocr: 'Kimi', ocr_fallback: false };
    } catch (e) {
      console.warn(`[ocr] Kimi falló (${e.message}), intentando con Gemini...`);
      if (process.env.GEMINI_API_KEY) {
        const resultado = await extraerConGemini(rutaArchivo, mimeType, prompt);
